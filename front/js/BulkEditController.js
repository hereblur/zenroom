...
.controller('BulkEditController', ['$scope', 'BookingsFactory', 'Roomtypes', '$rootScope', '$timeout', function($scope, BookingsFactory, Roomtypes, $rootScope, $timeout) {
    ...
    $scope.bulkSave = function(){
      ...
      BookingsFactory.save({
          roomtype: $scope.roomtype,
          date : [moment($scope.startDate).format("Y-MM-DD"),moment($scope.endDate).format("Y-MM-DD")].join('_'),
      }, {
          weekdays : weekdays,
          price : $scope.enableNewPrice ? $scope.newPrice : null,
          inventory : $scope.enableNewAvail ? $scope.newAvailability : null,
      }, function(data){
          $scope.saved = (data.length + " days has been successfully updated!")
          $scope.reset();
          ...
          $scope.extraError = ( response.data.error || Object.values(response.data)[0].join('<br>') || response.statusText )
      })
